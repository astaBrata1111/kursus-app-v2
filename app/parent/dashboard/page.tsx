...
export default function ParentDashboard() {
    ...
    const loadChildData = async (kid: any) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            const { data: profile } = await supabase.from('user_profiles').select('email, phone').eq('id', user.id).single();

            // Find students linked to this parent's phone/email
            const { data: kids } = await supabase.from('students').select('*')
                .or(`email.eq.${profile?.email},telepon.eq.${profile?.phone}`);
    ...
    const hadirPct = absensi.length > 0 ? Math.round((absensi.filter(a => a.status?.toLowerCase() === 'hadir').length / absensi.length) * 100) : 0;
        ...
        <div className="space-y-6">
            <div>
                <h1 style={{ fontWeight: 900, fontSize: '1.5rem', color: 'var(--text-primary)' }}>
                    {t('role_parent')} Portal 👪
                </h1>
                <p style={{ color: 'var(--text-muted)', fontSize: '0.875rem', marginTop: '0.25rem' }}>
                    {t('col_class') /* example fallback until specific term, or general info text */}
                </p>
            </div>
                ...
                <>
                    ...
                    <div className="card-amber p-6 rounded-2xl">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-white/70 text-sm font-semibold">{t('nav_data_murid')}</p>
                                ...
                                <div className="flex gap-2 mt-1">
                                    <span style={{ background: 'rgba(255,255,255,0.2)', color: 'white', fontSize: '0.75rem', fontWeight: 700, padding: '2px 10px', borderRadius: '99px' }}>
                                        {selected.level || t('badge_general')}
                                    </span>
                                    {selected.usia && (
                                        <span style={{ background: 'rgba(255,255,255,0.2)', color: 'white', fontSize: '0.75rem', fontWeight: 700, padding: '2px 10px', borderRadius: '99px' }}>
                                            {selected.usia} Tahun
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="card p-5">
                            <CreditCard size={22} style={{ color: pendingInvoices.length > 0 ? 'var(--danger)' : 'var(--success)', marginBottom: '0.5rem' }} />
                            <p style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>Tagihan Pending</p>
                            <p style={{ fontSize: '1.75rem', fontWeight: 900, color: pendingInvoices.length > 0 ? 'var(--danger)' : 'var(--success)' }}>
                    ...
                    {pendingInvoices.map(inv => (
                        <div key={inv.id} className="flex items-center gap-3 p-4 rounded-2xl" style={{ background: 'var(--danger-bg)', border: '1.5px solid rgba(214,106,106,0.35)' }}>
                    ...
                    <div className="card overflow-hidden">
                        <div className="p-5 border-b" style={{ borderColor: 'var(--border-light)' }}>
                            <h3 style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{t('nav_tagihan')}</h3>
                        </div>
